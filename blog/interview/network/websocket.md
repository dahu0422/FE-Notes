...
上述场景有一个共同特点——**实时性**，这种对实时性有要求的页面，会带来一些问题。比如下面的聊天场景
...
由于网络情况连接可能会意外中断，可以通过**心跳机制**恢复连接。

心跳机制通过定期发送心跳包来确保连接的活跃性。如果在预定时间内没有心跳响应，则认为连接断开。

连接断开后，客户端设置重连间隔，比如每间隔 30s 重新连接，如果超过则断开再重新发起。
