...
***ES6* 之前 *JavaScript* 没有块级作用域，只有全局作用域和函数作用域**。*ES6* 新增了 *let* 和 *const* 创建 “块级作用域”。
...
万万没想到，点击任意一个按钮，后台都是弹出“第四个”。这是因为 *i* 是全局变量，执行到点击事件时，此时 *i* 的值为 *3*。
...
自由变量的值如何得到 ？需要向父级作用域寻找（注意：这种说法并不严谨，下文会重点解释）。
...
**要到创建这个函数的作用域中取值，这里强调的是“创建”，而不是“调用”**，切记切记，其实这就是所谓的"静态作用域"。
